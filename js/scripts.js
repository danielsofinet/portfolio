...
---------------------------------------------------*/			
	...
	function InitShuffleElements() {	
		...
		shuffleFunctions.applyShuffleEffect(".showcase-portfolio .sofinet-item .slide-inner", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
		
		shuffleFunctions.applyShuffleEffect(".showcase-gallery .sofinet-slide", {
			velocity: 30,
			shuffleIterations: 8,
			childSelector: '.slide-title span, .slide-cat span'
		});
...
---------------------------------------------------*/	
	...
	function CleanupBeforeAjax(){		
		...
		sofinetSlider.instances.forEach(slider => slider.off());
		sofinetSlider.instances = [];
	}
...
---------------------------------------------------*/
	...
	function PageLoad() {		
		...
		gsap.set($(".sofinet-header"), {yPercent:-50, opacity:0});
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function() {
				...
				gsap.to($(".sofinet-header"), { duration: 0.45, opacity:1, yPercent:0, delay:0.25, ease: Power2.easeOut, });
	...
	function PageLoadActions() {
		...
		if (!isMobile()) {
			...
			$("#page-nav .next-ajax-link-page").on('mouseenter', function() {
				...
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
				$("#ball").addClass("with-blur");
				$( "#ball" ).append( '<p class="center-first" style="color:#2A2827">' + $this.data("centerline") + '</p>' );
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {	
				...
				$('.sofinet-header').removeClass('white-header');
				$("#app").remove();
				...
				gsap.to($("#main-page-content, #hero, .sofinet-footer, .sofinet-footer-nav"), {duration: 0.3, opacity:0, ease:Power4.easeOut});
				
				if ($("#page-nav").hasClass("move-nav-onload")) {
					$("body").addClass("load-next-page");
					if ($("body").hasClass("smooth-scroll")) {
						var moveNav = $("#content-scroll").height() - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".sofinet-footer").height() / 2	
					} else {
						var moveNav = window.innerHeight - ( $("#hero").height() / 2 )  - ( $("#page-nav").height()  ) / 2 - $(".sofinet-footer").height() / 2	   
					}
			...
			$('#page-nav .next-ajax-link-page').on('click', function() {					
				...
				gsap.to($(".sofinet-footer, .sofinet-footer-nav"), {duration: 0.3, opacity:0, delay:0, ease:Power2.easeInOut});
			});
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#project-nav").hasClass("auto-trigger")) {
				...
				gsap.to($("#project-nav"), {
					scrollTrigger: {
						trigger: $("#project-nav"), // Elementul care declanÈ™eazÄƒ efectul
						start: () => startValue,
						toggleClass: "active-link", // Clasa pe care o adaugÄƒ È™i o È™terge
					}
				});
			...
			$('.next-ajax-link-project').on('click', function() {
				...
				if ($(".sofinet-header").hasClass("swapped-logo")) {
					...
					gsap.to($("#sofinet-logo"), {duration: 0.2, opacity:0, onComplete: function() {
						imgLogoWhite.src = originalSrcWhite;
						imgLogoBlack.src = originalSrcBlack;
						gsap.to($("#sofinet-logo"), {duration: 0.2, opacity:1});
					}});
				...
				gsap.to($(".sofinet-footer, .all-works"), {duration: 0.3, opacity:0, ease:Power2.easeInOut});
		...
		if( $('#project-nav').length > 0 ){		
			...
			const pageContent = document.getElementById("sofinet-page-content");
			...
			ScrollTrigger.create({
				...
				onLeaveBack: function() {
					...
					gsap.set("#sofinet-logo img, .classic-menu .flexnav li, .button-wrap.menu, .button-icon-link", { clearProps: "all" });
				},			
				scrub: true,
			});	
				...
				if (changeMainColor) {
					...
					imagesLoaded('body', function() {
						...
						if (pageContent.classList.contains('light-content')) {
							...
							startLinkColor = "#F0DED7";
							endLinkColor = "#2A2827";						
							startButtonColor = "#F0DED7";
							endButtonColor = "#2A2827";
							...
							startLinkColor = "#2A2827";
							endLinkColor = "#F0DED7";
							
							startButtonColor = "#2A2827";
							endButtonColor = "#F0DED7";						
						...
						const blackLogoOpacity = gsap.getProperty('#sofinet-logo img.black-logo', 'opacity');
						const whiteLogoOpacity = gsap.getProperty('#sofinet-logo img.white-logo', 'opacity');
						...
						ScrollTrigger.create({
							...
							onEnter: function( st ) {
								...
								gsap.fromTo('#sofinet-logo img.black-logo', {
								...
								gsap.fromTo('#sofinet-logo img.white-logo', {
									opacity: whiteLogoOpacity
								}, {
									duration: 1, 
									opacity: blackLogoOpacity,
...
---------------------------------------------------*/	
	...
	function ShowcasePortfolio() {
		...
		gsap.utils.toArray('.sofinet-item .pixels-cover').forEach((pixelWrapper, index) => {
			const pixelAnimation = pixelWrapper.querySelectorAll(".pixel");
			...
			gsap.to(pixelAnimation, {				
				...
				opacity: 0, 
				delay: function() {
					// GenerÄƒm un delay random Ã®ntre 0 È™i 0.5 secunde
		...
		if( $('.showcase-portfolio').length > 0 ){
			...
			const pageContent = $('#sofinet-page-content');
			...
			document.getElementById('main').addEventListener('click', function (e) {
				if (e.target.closest('.show-filters') || e.target.id === 'close-filters') {
					var showFilters = document.querySelector('.show-filters'); // SelectÄƒm butonul principal
					var span = showFilters.querySelector('.button-text span');
					var icon = showFilters.querySelector('.button-icon i');
			
					// VerificÄƒm starea curentÄƒ È™i alternÄƒm
					if (span.getAttribute('data-temp') === 'Close') {
						// Revine la starea iniÈ›ialÄƒ
			...
			function filter() {				
				...
				var state = Flip.getState('.sofinet-item');
				var projects = document.querySelectorAll('.sofinet-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");
				
				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .sofinet-item.vertical-parallax .slide-inner');
			...
			if (!isMobile()) {	
				...
				$(".showcase-portfolio .sofinet-item .slide-inner").on('mouseenter', function() {
					$('#ball p').remove();
					var $this = $(this);			
...
---------------------------------------------------*/
	...
	function ShowcaseHighlights() {
		...
		if( $('.highlights-gallery').length > 0 ){
			...
			var slideElements = $(".sofinet-slide");
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					let firstSlide = document.querySelector(".sofinet-slide");
			...
			slider = new sofinetSlider('.highlights-gallery', {
				ease: 0.05,  
				direction: 'vertical', 
				snap: true,
				outer: '.sofinet-slider',		
				inner: '.sofinet-slider-viewport',	
				webgl: true,
				...
				on: {	
					init : function(slide) {
						...
						if (firstSlide.classList.contains("change-header")) {
							...
							gsap.to('#sofinet-logo img.black-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });	
							gsap.to('#sofinet-logo img.white-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });						
							gsap.to('.classic-menu .flexnav li', { duration: 0.3, color: "#2A2827", ease:Power2.easeInOut });
							gsap.to('header .button-wrap.menu', { duration: 0.3, color: "#2A2827", ease:Power2.easeInOut });	
							gsap.to('header .button-icon-link', { duration: 0.3, color: "#2A2827", boxShadow: "inset 0 0 15px rgba(0,0,0,0.3)", ease:Power2.easeInOut });
							gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.3, color: "#2A2827", ease:Power2.easeInOut });	
							gsap.to('.progress-info', { duration: 0.3, filter: 'invert(1)', ease:Power2.easeInOut });					
						} else {
							
							gsap.to('#sofinet-logo img.black-logo', { duration: 0.3, opacity: 0, ease:Power2.easeInOut });	
							gsap.to('#sofinet-logo img.white-logo', { duration: 0.3, opacity: 1, ease:Power2.easeInOut });						
							gsap.to('.classic-menu .flexnav li', { duration: 0.3, color: "#F0DED7", ease:Power2.easeInOut });
							gsap.to('header .button-wrap.menu', { duration: 0.3, color: "#F0DED7", ease:Power2.easeInOut });
							gsap.to('header .button-icon-link', { duration: 0.3, color: "#F0DED7", boxShadow: "inset 0 0 15px rgba(255,255,255,0.3)", ease:Power2.easeInOut });
							gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.3, color: "#F0DED7", ease:Power2.easeInOut });
			...
			function animateSlides(slider, arrElements, type = "translate", durationMultiplier = 1) {
				...
				for (let i = 0; i < arrElements.length; i++) {
					...
					if (type === "opacity") {
						// AnimaÈ›ie bazatÄƒ pe opacitate
						if (i === 0) {
							...
							gsap.set(slideElement, { opacity: 0 }); // Invizibil la Ã®nceput
						...
						// AnimaÈ›ie bazatÄƒ pe yPercent
						if (i === 0) {
							slider.tl.to(slideElement, { yPercent: 0, duration: staticDuration * 0.5 }, 0) // ÃŽn view
									 .to(slideElement, { yPercent: -100, duration: transitionDuration }, '>'); // Fade out
						} else {
							gsap.set(slideElement, { yPercent: 100 }); // ÃŽn afara ecranului
				...
				// GestionÄƒm primul slide separat, dacÄƒ este necesar
			...
			// ApelÄƒm funcÈ›ia pentru fiecare set de elemente
			const arrTitles = gsap.utils.toArray('.sofinet-caption-wrapper .external-titles .slide-title');
			const arrCategories = gsap.utils.toArray('.sofinet-caption-wrapper .external-categories .slide-cat');
			...
			if (!isMobile()) {
				...
				$('.sofinet-slider').on('mousedown', function (evt) {
					$('.sofinet-slider').on('mouseup mousemove', function handler(evt) {
						if (evt.type === 'mouseup') {					  
							// click
							gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
							...
							if ($('#magic-cursor').hasClass("light-content")) {
								gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#F0DED7', backgroundColor:'#F0DED7'});
							} else {
								gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#2A2827', backgroundColor:'#2A2827'});
							}
						...
						$('.sofinet-slider').off('mouseup mousemove', handler);
					});
				});
				
					
				$('.sofinet-slider').on('mouseup touchend', function() {
				...
				$(".external-titles .slide-title").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-y')) {
						...
						let relatedSlide = $('.sofinet-slide[data-slide="' + dataCaption + '"]');
			...
			$('.external-titles .slide-title').on('click', function () {
				...
				let dataCaption = $(this).attr('data-caption'); 
				let parentSlide = $('.sofinet-slide[data-slide="' + dataCaption + '"]');
				...
				gsap.to('.sofinet-caption-wrapper', { duration: 0.5, opacity: 0, filter: "blur(5px)", ease: Power4.easeInOut });
...
---------------------------------------------------*/
	...
	function ShowcaseGallery() {
		...
		if( $('.showcase-gallery').length > 0 ){
			...
			gsap.set($(".showcase-gallery .sofinet-slider .slide-effects"), { opacity: 0 });
			...
			if (!$('body').hasClass("show-loader")) {
				...
				setTimeout(function() {
					...
					gsap.to($(".showcase-gallery .sofinet-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 1.1, ease: Power4.easeIn, onComplete: function() {
						$(".showcase-gallery").addClass("active");
					}});
					
					var gallerySlideClasses = [".sofinet-slide-prev", ".sofinet-slide-active", ".sofinet-slide-next"];
					gallerySlideClasses.forEach(function(gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .sofinet-slider " + gallerySlideClass + " .slide-effects");
			...
			const slidesRoot = document.querySelector('.sofinet-sync-slider .sofinet-sync-slider-wrapper .sofinet-sync-slider-viewport');
			const slidesList = slidesRoot.querySelectorAll('.sofinet-sync-slider .sofinet-sync-slide');
			
			let slidesHeight = 0;
			
			for (const sofinetSyncSlide of slidesList) {
				slidesHeight += sofinetSyncSlide.offsetHeight;
			}
			...
			if (iterCloning >= 1) {
				...
				for (let i = 0; i < iterCloning; i++) {
					for (const sofinetSyncSlide of slidesList) {
						let cloneSlide = sofinetSyncSlide.cloneNode(true);
						slidesRoot.appendChild(cloneSlide);
					}
				}
			}
			
			const sofinetSyncSlider = document.querySelector('.sofinet-slider-wrapper.showcase-gallery .sofinet-sync-slider-viewport');
			const syncSliderClone = sofinetSyncSlider.cloneNode(true);
			document.querySelector(".sofinet-slider-wrapper.showcase-gallery .sofinet-sync-slider-wrapper").appendChild(syncSliderClone);

			
			
			
			slider = new sofinetSlider('.sofinet-slider-wrapper', { 
				...
				on: {	
					init : function(slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function() {
								...
								gsap.to($(".showcase-gallery .sofinet-slide .slide-effects"), { duration: 0.5, opacity: 1, delay: 0.7, ease: Power4.easeIn, onComplete: function() {
									$(".showcase-gallery").addClass("active");
								}});
					...
					slideLeaveViewport : function( slide ) {
						gsap.set($('.sofinet-slider div:not(.sofinet-slide-visible) .slide-effects'), { x: "" });		
					},
				},
			});			
			
			const syncSliderCloneTranslate = document.querySelectorAll('.sofinet-slider-wrapper.showcase-gallery .sofinet-sync-slider-wrapper > .sofinet-sync-slider-viewport');
			const titleWrapper = document.querySelector('.sofinet-sync-slider-wrapper');
			
			slider.tl
				.fromTo('.sofinet-slider-wrapper.showcase-gallery .sofinet-sync-slider-wrapper', {yPercent: 0 },{ yPercent: -(100 - (100 / syncSliderCloneTranslate.length))},0)
				.fromTo('.hover-reveal', {y: 0}, {y: + (titleWrapper.offsetHeight/2)}, 0)
    

			//Slider Hover Events
			
			$('.sofinet-sync-slide').on('mouseenter', function() {
				$('.sofinet-sync-slide').addClass('disable'); 
				$(this).removeClass('disable').addClass('active'); 
			}).on('mouseleave', function() {
				$('.sofinet-sync-slide').removeClass('disable active'); 
			});
			...
			Array.from(document.querySelectorAll('.sofinet-sync-slide')).forEach(link => new HoverImgFx(link));
			...
			let gridPreview = new sofinetGridPreview( slider, {
			...
			gridPreview.on.beforeShowGrid = () => {
				...
				gsap.to($(".sofinet-slide.hovered .slide-caption span"), {duration: 0.15, y:-30, opacity:0, stagger:0.05, ease:Power2.easeIn});
				gsap.to($(".sofinet-slide .slide-caption"), {duration: 0.2, opacity:0, delay:0, ease:Power2.easeIn});
				gsap.to($(".sofinet-slide .slide-thumb img"), {duration: 0.3, scale:0, opacity:0, ease:Power2.easeIn, onComplete: function() {
					gsap.set(".sofinet-slide .slide-thumb", { scale: 0, opacity:0 });
				}});
			...
			gridPreview.on.flipShowGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				gsap.to(".sofinet-slider .sofinet-slide .trigger-item", { duration: 1, opacity:0, scale:0.7, ease:Power2.easeOut });
				...
				if (document.getElementById('sofinet-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#sofinet-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#sofinet-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });						
						gsap.to('.classic-menu .flexnav li', { duration: 0.5, delay:0.15, color: "#F0DED7", ease:Power2.easeInOut });
						gsap.to('header .button-wrap.menu', { duration: 0.5, delay:0.15, color: "#F0DED7", ease:Power2.easeInOut });
						gsap.to('header .button-icon-link', { duration: 0.5, delay:0.15, color: "#F0DED7", boxShadow: "inset 0 0 15px rgba(255,255,255,0.3)", ease:Power2.easeInOut });
						gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.5, delay:0.15, color: "#F0DED7", ease:Power2.easeInOut });					
					}
				} else if (document.getElementById('sofinet-page-content').classList.contains('light-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#sofinet-logo img.black-logo', { duration: 0.5, delay:0.15, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#sofinet-logo img.white-logo', { duration: 0.5, delay:0.15, opacity: 0, ease:Power2.easeInOut });						
						gsap.to('.classic-menu .flexnav li', { duration: 0.5, delay:0.15, color: "#2A2827", ease:Power2.easeInOut });
						gsap.to('header .button-wrap.menu', { duration: 0.5, delay:0.15, color: "#2A2827", ease:Power2.easeInOut });	
						gsap.to('header .button-icon-link', { duration: 0.5, delay:0.15, color: "#2A2827", boxShadow: "inset 0 0 15px rgba(0,0,0,0.3)", ease:Power2.easeInOut });
						gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.5, delay:0.15, color: "#2A2827", ease:Power2.easeInOut });								
					}
				}
			...
			gridPreview.on.beforeHideGrid = () => {
				...
				gsap.to(".sofinet-slider .sofinet-slide .trigger-item", { duration: 0.5, opacity:1, scale:1, delay:0.2, ease: 'power3.inOut' });
				...
				gsap.set($(".sofinet-slide.hovered .slide-caption span"), {y: 30 });				
				gsap.to($(".sofinet-slide.hovered .slide-caption span"), {
					duration: 0.5,
					y: 0,
					opacity: 1,
					delay: 0.5,
					...
					ease: Power2.easeOut,
					onComplete: function() {
						gsap.set($(".sofinet-slide .slide-caption span"), { clearProps: "all" });
					}
				});
				gsap.to($(".sofinet-slide .slide-caption"), {duration: 0.3, opacity:1, delay:0.5, ease:Power2.easeIn});
				gsap.set(".sofinet-slide .slide-thumb img", { scale: 1, opacity:1, onComplete: function() {
					gsap.to($(".sofinet-slide .slide-thumb"), {duration: 0.5, scale: 1, delay:0.5, opacity:1, ease: "back.out(2)" });
				}});
			...
			gridPreview.on.flipHideGrid = (flipstateAllImages, flipstateCurrentImage) => {
				...
				Flip.from(flipstateCurrentImage, {
					...
					onComplete: function() {
						...
						const triggeredItem = document.querySelector('.sofinet-slide.triggered-item');
						if( triggeredItem != null ){
							triggeredItem.classList.remove('triggered-item');
						}
						
						const sofinetSlides = document.querySelectorAll('.sofinet-slide');
						sofinetSlides.forEach(slide => {
						  	slide.style.zIndex = '';
				...
				gsap.to('.header-gradient', { duration: 0.4, backgroundColor: $("#sofinet-page-content").data("bgcolor"), ease:Power2.easeInOut });
				gsap.to("main", { duration: 0.4, backgroundColor: $("#sofinet-page-content").data("bgcolor"), ease: Power2.easeInOut });
				
				
				if (document.getElementById('sofinet-page-content').classList.contains('dark-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {
						gsap.to('#sofinet-logo img.black-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });	
						gsap.to('#sofinet-logo img.white-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });						
						gsap.to('.classic-menu .flexnav li', { duration: 0.4, color: "#2A2827", ease:Power2.easeInOut });
						gsap.to('header .button-wrap.menu', { duration: 0.4, color: "#2A2827", ease:Power2.easeInOut });							
						gsap.to('header .button-icon-link', { duration: 0.3, color: "#2A2827", boxShadow: "inset 0 0 15px rgba(0,0,0,0.3)", ease:Power2.easeInOut });
						gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.4, color: "#2A2827", ease:Power2.easeInOut });						
					}
				} else if (document.getElementById('sofinet-page-content').classList.contains('light-content')) {
					if (gridPreview.currentImage.classList.contains('change-header')) {						
						gsap.to('#sofinet-logo img.black-logo', { duration: 0.4, opacity: 0, ease:Power2.easeInOut });	
						gsap.to('#sofinet-logo img.white-logo', { duration: 0.4, opacity: 1, ease:Power2.easeInOut });						
						gsap.to('.classic-menu .flexnav li', { duration: 0.5, color: "#F0DED7", ease:Power2.easeInOut });	
						gsap.to('header .button-wrap.menu', { duration: 0.4, color: "#F0DED7", ease:Power2.easeInOut });							
						gsap.to('header .button-icon-link', { duration: 0.3, color: "#F0DED7", boxShadow: "inset 0 0 15px rgba(255,255,255,0.3)", ease:Power2.easeInOut });
						gsap.to('.button-icon-link.cp-button-prev, .button-icon-link.cp-button-next', { duration: 0.4, color: "#F0DED7", ease:Power2.easeInOut });									
					}
				}				
				
				
				gsap.to($("footer .link-text, .sofinet-pagination, .progress-info, #filters-wrapper"), {duration: 0.3, opacity:1, y:0, stagger:0.05, delay:0.4, ease:Power2.easeInOut});
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			gridPreview.on.navigate = (flipstate, flipstateThumbs) => {
				...
				Flip.from(flipstate, {
						...
						absolute: true,
						onComplete: function() {							
							gsap.to(".showcase-gallery .gallery-zoom-wrapper", { duration: 0.3, opacity:1, scale:1, delay:0, ease:Power2.easeOut });
						}					 
				});
			...
			if (!isMobile()) {
				...
				$('.sofinet-slide').on('mouseenter', function() {
					$('.sofinet-slide').removeClass('hovered');
					$(this).addClass('hovered');
				});
					
				
				$(".showcase-gallery.preview-mode-enabled .sofinet-slide .slide-inner-height").on('mouseenter', function() {	
				...
				$(".showcase-gallery .sofinet-sync-slide .trigger-item").on('mouseenter', function() {	
					if (!$('body').hasClass('scale-drag-x')) {
						$('#ball p').remove();
						var $this = $(this);			
						gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.4, borderColor:"rgba(170,170,170,0)", backgroundColor:"rgba(170,170,170,0.3)"});
			...
			// IniÈ›ializeazÄƒ poziÈ›ia È™i dimensiunea lui .current-option
			updateCurrentOption();
			
			// FuncÈ›ie pentru gestionarea clasei `disable`
			function setDisableState(state) {
				const $parent = $('.grid-list-option');
				if (state) {
					$parent.addClass('disable'); // AdaugÄƒ clasa disable
				} else {
					$parent.removeClass('disable'); // EliminÄƒ clasa disable
				}
			}
			...
			$('.grid-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					setDisableState(true); // AdaugÄƒ clasa `disable` la Ã®nceput
			
					$(".sofinet-sync-slider").removeClass("active");
			
					gsap.to($(".showcase-gallery .sofinet-sync-slide .slide-title span"), {
						duration: 0.5,
						yPercent: -100,
						opacity: 0,
						stagger: 0.01,
					...
					gsap.to($(".showcase-gallery .sofinet-slide .slide-effects"), {
						duration: 0.5,
						opacity: 1,
						delay: 0.5,
						ease: Power4.easeIn,
					...
					gallerySlideClasses.forEach(function (gallerySlideClass, index) {
						...
						gsap.to(gallerySlide, {
							...
							onComplete: function () {
								...
								// CÃ¢nd toate animaÈ›iile sunt complete, eliminÄƒ clasa `disable`
								animationsCompleted++;
								if (animationsCompleted === gallerySlideClasses.length) {
									setDisableState(false); // EliminÄƒ clasa `disable`
								}
							}
						});
					});
			...
			$('.list-option').on('click', function () {
				...
				if (!$this.hasClass('active')) {
					...
					gsap.set($(".sofinet-sync-slide .slide-title span"), { yPercent: 100, opacity: 0 });
			
					gsap.to($(".sofinet-sync-slide .slide-title span"), {
						duration: 0.7,
						yPercent: 0,
						opacity: 1,
						stagger: 0.02,
					...
					setTimeout(function () {
						$(".sofinet-sync-slider").addClass("active");
					}, 1200);
				}
			});
