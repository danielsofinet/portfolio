...
---------------------------------------------------*/
	...
	window.ScrollEffects = function() {
		...
		function calculatePixels() {
			...
			pixelsCovers.forEach(function(pixelsCover) {
				// ObÈ›inem pÄƒrintele imediat al fiecÄƒrui .pixels-cover
				var parent = pixelsCover.closest('.content-row'); // CÄƒutÄƒm pÄƒrintele cel mai apropiat cu clasa .content-row
			
				if (parent) {
					// AdÄƒugÄƒm o clasÄƒ pÄƒrintelui (opÈ›ional)
					parent.classList.add('pixels-parent');
			
					// ObÈ›inem atributul 'data-bgcolor' al pÄƒrintelui
			...
			// VerificÄƒm dacÄƒ existÄƒ un pÄƒrinte .content-row pentru prima instanÈ›Äƒ a .pixels-wrapper
			...
			if (firstWrapper) {
				...
				if (parentRow) {
					...
					bgColor = document.getElementById('sofinet-page-content').getAttribute('data-bgcolor');
				}
			} else {
				bgColor = document.getElementById('sofinet-page-content').getAttribute('data-bgcolor');
			}
		...
		// Call functions
		// VerificÄƒm dacÄƒ body nu are clasele 'load-project-thumbs' È™i 'load-page'
		...
		if (isMobile()) {
			...
			function setViewPortHeight() {
				...
				gsap.set(".smooth-scroll main, .has-parallax, .sofinet-slider-wrapper:not(.content-slider), .showcase-lists .sofinet-sync-slider, .slider-fixed-content", { height: winHeight});
			}
		...
		if ($('body').hasClass("swap-logo")) {
			...
			ScrollTrigger.create({
				trigger: '.sofinet-header', // Set the trigger element
				start: 'top 120px',
				onEnter: function() {
				...
				onEnterBack: function() {
					...
					$('.sofinet-header').removeClass('swapped-logo');
				},
				onLeave: function() {
					...
					$('.sofinet-header').addClass('swapped-logo');			
				},
		...
		if( $('.new-hero-title').length > 0 ){
			...
			ScrollTrigger.create({
				...
				scrub: true,
				pinSpacing: window.innerWidth > 768 ? false : true, // Valoare dinamicÄƒ
			});
		...
		if ($("#hero-interaction").length) {
			...
			$(interactionTarget).mouseenter(function(e) {   
				...
				$("#ball").append('<img class="eyes-icon" src="images/smiley.svg" style="transform:scale(0.75);">');                
			
				let imageIndex = 0;
				const images = [
					'<img class="eyes-icon" src="images/smiley.svg" style="transform:scale(0.75);">',
					'<img class="eyes-icon" src="images/smiley.svg" style="transform:scale(0.75);">' 
				];
		...
		gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
			...
			function setClippedImageWrapperProperties() {
				...
				gsap.set(clippedImage, { height: window.innerHeight - document.querySelector('.sofinet-header').offsetHeight * 2 });								
				gsap.set(clippedImageContent, { paddingTop: (window.innerHeight/2) + clippedImageContent.offsetHeight});
				gsap.set(clippedImageWrapper, { height: window.innerHeight + clippedImageContent.offsetHeight - document.querySelector('.sofinet-header').offsetHeight});
		...
		if( $('.pinned-lists').length > 0 ){
			...
			wrappers.forEach(wrapper => {
				...
				if (wrapper.classList.contains('brick-mode')) {
					...
					lists.forEach((list, index) => {
						if (index === 0) {
							// Primul element are clasa 'secondary-font' È™i opacitatea 1
							gsap.set(list, {
						...
						if (list.hasAttribute('data-infoTextAfter')) {
							// AlternÄƒm clasele Ã®n funcÈ›ie de indexul curent
							if (infoIndex % 2 === 0) {
								list.classList.add('has-info-even');
							} else {
								list.classList.add('has-info-odd');
							}
							infoIndex++; // IncrementÄƒm contorul doar pentru elementele cu data-infoTextAfter
						}
						
					});
					
					// CreeazÄƒ timeline-ul pentru mutarea clasei .secondary-font È™i modificarea opacitÄƒÈ›ii
					lists.forEach((list, index) => {
						if (index !== 0) {
							// La fiecare pas, mutÄƒm clasa de la elementul anterior È™i setÄƒm opacitatea corespunzÄƒtoare
							tl.to(lists[index - 1], {
								onComplete: () => {
									// ÃŽndepÄƒrteazÄƒ clasa de la elementul anterior
									lists[index - 1].classList.remove('secondary-font');
									// SeteazÄƒ opacitatea la 1 pentru elementul anterior
									lists[index - 1].style.opacity = 1;
								},
								duration: durationHeight
							}).to(list, {
								onStart: () => {
									// AdaugÄƒ clasa la elementul curent
									list.classList.add('secondary-font');
									// SeteazÄƒ opacitatea la 1 pentru elementul curent
									list.style.opacity = 1;
								},
								onReverseComplete: () => {
									// La scroll Ã®napoi, mutÄƒm clasa È™i opacitatea
									if (index > 0) {
										lists[index].classList.remove('secondary-font');
										lists[index - 1].classList.add('secondary-font');
										// SeteazÄƒ opacitatea la 1 pentru elementul anterior
										lists[index - 1].style.opacity = 1;
										// SeteazÄƒ opacitatea la 0.2 pentru elementul curent
		...
		imagesLoaded('body', function() {
			...
			const staticDuration = 30; // Durata staticÄƒ a animaÈ›iei (viteza implicitÄƒ)
			const velocityPower = 5; // Puterea efectului velocity la scroll
			
			let direction = 1; // DirecÈ›ia animaÈ›iei (1 = Ã®nainte, -1 = Ã®napoi)
			
			// CreeazÄƒ animaÈ›iile de bazÄƒ
			...
			// SetÄƒm viteza staticÄƒ la Ã®nceput
			gsap.to([marqueeFw, marqueeBw], { timeScale: 1, overwrite: true });
			
			// ScrollTrigger pentru controlul vitezei dinamice
			const scroll = ScrollTrigger.create({
			  onUpdate(self) {
				const velocity = Math.abs(self.getVelocity() / 100); // ObÈ›inem velocity
				const newTimeScale = gsap.utils.clamp(1, 10, Math.pow(velocity, velocityPower)); // AplicÄƒm "puterea" la velocity
			
				if (self.direction !== direction) {
				  direction *= -1; // SchimbÄƒm direcÈ›ia
			  ...
			  onLeave() {
				// ResetÄƒm la viteza staticÄƒ dacÄƒ scroll-ul se opreÈ™te
				gsap.to([marqueeFw, marqueeBw], { timeScale: 1, overwrite: true });
			  }
			});
			
			// FuncÈ›ia pentru crearea efectului de rulare
		...
		function setMainPageContentMargins() {
			var winHeight = $(window).height();
			var footerHeight = $('.sofinet-footer').height();
		...
		gsap.to('.page-nav-caption', {
			...
			scrollTrigger: {
				trigger: '#page-nav',
				start: startValue,
				end: () => `+=${$('#page-nav').outerHeight() + $('.sofinet-footer').outerHeight()}`,
				scrub: true,
			}			
		});
			...
			imagesLoaded('body', function() {
				setTimeout(function() {
					...
					changeHeaderColor.forEach(function(changeHeaderElement) {						
						var pageHeader = document.querySelector('.sofinet-header');							
		...
		if ($('#sofinet-page-content').hasClass("light-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
						if (pageNav) {
							var pageContent = document.querySelector('#sofinet-page-content');	
		...
		if ($('#sofinet-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
		  		imagesLoaded('body', function() {
					setTimeout(function() {	
						var pageNav = document.querySelector('#project-nav.change-header');
		...
		gsap.set('.header-gradient', {
			delay: 0.5,
			backgroundColor: document.getElementById('sofinet-page-content').getAttribute('data-bgcolor'),			
			onUpdate: function() {
			var bgColor = document.getElementById('sofinet-page-content').getAttribute('data-bgcolor');
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to('#sofinet-logo img.white-logo, #sofinet-logo img.black-logo', { duration: 0.5, delay:0, opacity: "",  ease: Linear.easeNone, });
...
---------------------------------------------------*/	
	...
	window.FirstLoad = function() {
		...
		if ($("#sofinet-page-content").hasClass("light-content")) {
			$(".sofinet-nav-wrapper").css('background-color', function () {
				return $(".sofinet-header").data('menucolor')
			});
			
			gsap.to("main", {duration: 0.5, backgroundColor: document.querySelector("#sofinet-page-content").getAttribute("data-bgcolor"), ease:Power2.easeInOut});
			
			$('#magic-cursor').addClass('light-content');
			if( $('#hero').length > 0 ){						
				if( $('#hero').hasClass("has-image")) {	
					$(".sofinet-header").css('background-color', 'transparent');
				} else {
					if ($(".sofinet-header").hasClass("fullscreen-menu")) {
						$(".sofinet-header").css('background-color', 'transparent');
					} else {
						if( $('#blog').length > 0 ){
							$(".sofinet-header").css('background-color', '#171717');
						}
						if( $('#post').length > 0 ){
							$(".sofinet-header").css('background-color', '#171717');
						}
			...
			if( $('#hero').length > 0 ){	
				if( $('#hero').hasClass("has-image")) {	
					...
					if ($(".sofinet-header").hasClass("fullscreen-menu")) {
						...
						if( $('#blog').length > 0 ){
							$(".sofinet-header").css('background-color', '#F0DED7');
						}
						if( $('#post').length > 0 ){
							$(".sofinet-header").css('background-color', '#F0DED7');
						}
		...
		$('a.ajax-link').on('click', function() {
			...
			gsap.to("nav", {duration: 0.3, backgroundColor: document.querySelector("#sofinet-page-content").getAttribute("data-bgcolor")});
		...
		$('.sofinet-nav-wrapper .ajax-link').on('click', function() {
		...
		$('#burger-wrapper, .menu .button-text').on('click', function() {
			$('#menu-burger, .sofinet-nav-wrapper').toggleClass('open');			
			setTimeout( function(){			
				if ($('#menu-burger').hasClass("open")) {
					
					gsap.to('.sofinet-nav-wrapper', {duration: 0.3, opacity:1, ease:Power2.easeInOut});
					
					$('.sofinet-header').addClass('over-sidebar').addClass('over-white-section');
					if (!$('#page-content').hasClass("light-content")) {	
						$('#magic-cursor').addClass('light-content');
					}
					if ($('.sofinet-header').hasClass("invert-header")) {
						$('#header-container').addClass('light-content-header');
					} else {
						$('#header-container').addClass('dark-content-header');
					}
					gsap.set($(".sofinet-nav-wrapper ul ul li"), {y: 0, opacity:1});
					...
					gsap.to('.sofinet-nav-wrapper', {duration: 0.3, opacity:0, delay:0.6, ease:Power2.easeInOut});
						
					//Fade Out Navigation Lists						
					gsap.to($(".menu-timeline .before-span"), {duration: 0.5, y:-200, opacity:1, delay:0, stagger:0.05, ease:Power2.easeIn});
					gsap.to($(".sofinet-nav-wrapper ul ul li"), {duration: 0.5, y:-120, opacity:0, delay:0, stagger:0.03, ease:Power2.easeIn});
					...
					setTimeout( function(){
						...
						$('.sofinet-header').removeClass('over-sidebar')
						setTimeout( function(){
							$('.sofinet-header').removeClass('over-white-section');
						} , 350 );
					} , 500 );
				}							
			} , 20 );
				  ...
				  $body.on('mouseup mousemove', function handler(evt) {
					if (evt.type === 'mouseup') {
					  ...
					  $('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function() {					
							...
							/*if (!$('#sofinet-page-content').hasClass("light-content")) {
								...
								if (!$('.portfolio').hasClass("portfolio-shortcode")) {
									if (!parent_item.hasClass("change-header")) {
										$('#sofinet-page-content').delay(700).queue(function(next){							
							...
							$('.sofinet-slider-wrapper .trigger-item').each(function(){
							...
							gsap.to('#hero, #show-filters, .item-caption-wrapper, .showcase-portfolio .slide-caption, #page-nav, .sofinet-footer, .fadeout-element', {duration: 0.5, opacity:0, ease:Power4.easeInOut});
...
---------------------------------------------------*/
	...
	window.Shortcodes = function() {  	
		...
		$('#copy-email').on('click', function() {
			...
			// VerificÄƒm dacÄƒ navigator.clipboard este disponibil
			if (navigator.clipboard && navigator.clipboard.writeText) {
				navigator.clipboard.writeText(email).then(function() {
					$('#ball p').remove(); 
					$("#ball").append('<p class="center-first" style="color:#2A2827">' + clickedMessage + '</p>'); 
				}).catch(function(err) {
					$("#ball p").remove();
					$("#ball").append('<p class="center-first" style="color:#2A2827">Failed to copy email!</p>');
					console.error('Error copying email: ', err);
				});
			} else {
				// Fallback: creÄƒm un textarea pentru a copia emailul
				var $temp = $('<textarea>');
				$('body').append($temp);
		...
		$("#copy-email").on('mouseenter', function() {
			...
			if (!$(this).hasClass('copied')) {
				...
				$("#ball").addClass("with-blur");
				$("#ball p").remove();
				$("#ball").append('<p class="center-first" style="color:#2A2827">' + $this.data("hover-message") + '</p>');
				...
				$("#ball").addClass("with-blur");
				$("#ball p").remove();
				$("#ball").append('<p class="center-first" style="color:#2A2827">' + $this.data("clicked-message") + '</p>');				
			}
...
---------------------------------------------------*/
	...
	window.Sliders = function() { 
		...
		if( $('.content-slider').length > 0 ){
			...
			slider = new sofinetSlider('.content-slider', { 
				...
				renderBullet: function (index, className) {
						return '<div class="parallax-wrap">' + '<div class="parallax-element">' + '<svg class="fp-arc-loader" width="20" height="20" viewBox="0 0 20 20">'+
									'<circle class="path" cx="10" cy="10" r="5.5" fill="none" transform="rotate(-90 10 10)" stroke="#F0DED7"'+ 'stroke-opacity="1" stroke-width="2px"></circle>' + '<circle class="solid-fill" cx="10" cy="10" r="3" fill="#F0DED7"></circle>' + '</svg></div></div>';
				},
			});
			
			$(".slider-button-prev").mouseenter(function(e) {				
				var modifyMouseColor = $('#sofinet-page-content').attr("data-modify-color");
				if (modifyMouseColor) {
					...
					if ($(this).parents(".sofinet-slider-wrapper").hasClass("light-cursor")) {
						$("body").addClass("drag-cursor-white");
						gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#F0DED7', backgroundColor:'#F0DED7'});
					} else if ($(this).parents(".sofinet-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .sofinet-slider, .content-slider.small-looped-carousel .sofinet-slider').on('mouseenter mousemove', function() {	
...
---------------------------------------------------*/
	...
	window.Lightbox = function() {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="sofinet-img-popup">
				<div class="sofinet-img-popup-bg-close"></div>				
				<div class="sofinet-img-popup-viewport">
					<div class="sofinet-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="sofinet-img-popup-close link"></div>						
				</div>				
				<div class="sofinet-img-popup-prev link"></div>
				<div class="sofinet-img-popup-next link"></div></div>`);
			
			const details = document.querySelector('.sofinet-img-popup');
			const detailsBgClose = document.querySelector('.sofinet-img-popup-bg-close');
			const detailsClose = document.querySelector('.sofinet-img-popup-close');
			const detailsPreloader = document.querySelector('.sofinet-img-popup-preloader');
			const detailImage = document.querySelector('.sofinet-img-popup img');
			const detailPrev = document.querySelector('.sofinet-img-popup-prev');
			const detailNext = document.querySelector('.sofinet-img-popup-next');
		...
		$(".image-link").mouseenter(function(e) {	
			...
			if (modifyMouseColor) {
				...
				gsap.to('#ball', {duration: 0.2, borderWidth: '2px', scale: 1, borderColor:'#F0DED7'});
			}
			gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="sofinet-video-popup">
				<div class="sofinet-video-popup-bg-close"></div>				
				<div class="sofinet-video-popup-viewport">
					<div class="sofinet-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="sofinet-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="sofinet-video-popup-close link"></div>						
				</div>				
				<div class="sofinet-img-video-prev link"></div>
				<div class="sofinet-img-video-next link"></div></div>`);
			
			const details = document.querySelector('.sofinet-video-popup');
			const detailsBgClose = document.querySelector('.sofinet-video-popup-bg-close');
			const detailsClose = document.querySelector('.sofinet-video-popup-close');
			const detailsPreloader = document.querySelector('.sofinet-video-popup-preloader');
			const detailIframe = document.querySelector('.sofinet-video-popup iframe');
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			function updatePosition() {
			  if (!active) {
				...
				// AplicÄƒm blur-ul doar dacÄƒ este activat
				if (isBlurEnabled) {
				  ...
				  // DacÄƒ blur-ul este dezactivat, resetÄƒm la 0
				  gsap.to(ball, {
					duration: 0.1,
					filter: `blur(0px)`
				  });
			...
			$(".sofinet-button .sticky.left").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
				var positionX = rcBounds.left  + 22;
				var positionY = rcBounds.top + rcBounds.height/2;		  
				gsap.to(ball, { duration: 0.5, x: positionX, y: positionY, scale:0.4, opacity:1, borderWidth: '6px', borderColor:'#2A2827'});
				gsap.ticker.remove(updatePosition);
			})
			
			$(".sofinet-button .sticky.right").mouseenter(function(e) {		  
				var rcBounds = $(this)[0].getBoundingClientRect();		  
			...
			$(".sofinet-button .parallax-wrap.icon-wrap").mouseenter(function(e) {
				gsap.to(ball, { duration: 0.05, scale: 0.4, borderWidth: '0px', opacity:1, borderColor:'#2A2827' });
			});
			...
			$(".sofinet-button .sticky").mouseenter(function(e) {
				if ($('#sofinet-page-content').hasClass("light-content")) {
					gsap.to(ball, { duration: 0.5, borderColor:'#2A2827' });
				} else {
					gsap.to(ball, { duration: 0.5, borderColor:'#F0DED7' });  
				}
			});
			
			$(".sofinet-button .parallax-wrap").mouseenter(function(e) {
				if ($('#sofinet-page-content').hasClass("light-content")) {
					gsap.to(ball, { duration: 0.05, borderColor:'#2A2827'  });
				} else {
					gsap.to(ball, { duration: 0.05, borderColor:'#F0DED7'  });
				}
			});
			$("#main .parallax-wrap.icon-wrap").mouseenter(function(e) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .sofinet-sidebar-widget a").mouseenter(function(e) {	
				gsap.to('#ball', {duration: 0.2, borderWidth: '1px', scale: 1, opacity:0});
			});	
					
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .sofinet-sidebar-widget a").mouseleave(function(e) {
				gsap.to('#ball', {duration: 0.3, borderWidth: '4px', scale:0.5, opacity:1});
			});
		}
			  ...
			  $(window).on('popstate', function() {
				...
				section.load(url+' .cd-main-content > *', function(event){
				  // load new content and replace <main> content with the new one
				 	...
				 	var sofinet_title = event.match(/<title[^>]*>([^<]+)<\/title>/)[1];
					$('head title').html( sofinet_title );
